...
uint8_t get_rode_type()
{
	...
	{
		return IsOutOfRoad()?Outside:IsStartLine(pre_sight+5)?Startline:IslandfristEntrancejudge()?Island_in:IsCrossRoad()?CrossRoad:IsStraightLine()?Straight:WhichCurve() ;	 
	}
	else if((LeftIsland.f_IslandIn||RightIsland.f_IslandIn)&&IslandStep==3)//识别到环岛入口，准备识别第二个入口
	{
//				OLED_Fill(0);
//		OLEDPrintf(0,7, "10") ;
		return IsOutOfRoad()?Outside:IslandsecondEntrancejudge()?Island_SecondEntrance:FALSE;
...
//				OLED_Fill(0);
//		OLEDPrintf(50,7, "50") ;
		return IsOutOfRoad()?Outside:IslandExitjudge()?Island_out:FALSE;
	}
