...
int16 MT9V032_CFG[CONFIG_FINISH][2]=
{
    ...
    {UD_OFFSET,         -140},   //图像上下偏移量    正值 上偏移   负值 下偏移  行为120 240 480时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
...
void camera_init(void)
{
    ...
    dma_portx2buff_init(DMA_CH0, (void *)&D_IN_DATA(0), (void *)image, C13, DMA_BYTE1, COL*ROW, DADDR_KEEPON);
    port_init(C13, ALT1 | DMA_RISING | PULLDOWN);  			    //PCLK  触发源设置
    ...
    port_init(C18, ALT1 | IRQ_FALLING | PULLDOWN);        		//场中断，下降沿触发中断、下拉
...
void seekfree_sendimg_032(void)
{
    ...
    uart_putbuff(uart3, (uint8_t *)image, ROW*COL);  //发送图像image_binary
}
...
void translate_image(){
  static int i=0,j=0;
	...
	{		
		for(j=0;j<=COL-1;j++)	
     if((j==resultSet.leftBorder[i]+1)||(j==resultSet.rightBorder[i]-1) ||(j==resultSet.middleLine[i]))
			 if(j==resultSet.middleLine[i]){
				  uart_putchar(uart1,0x00);
			 }
			  else  
					uart_putchar(uart1,0xa0);
	     else{
		    if(image_binary[i][j]==0x00)
			     uart_putchar(uart1,0x00); 
	       else
